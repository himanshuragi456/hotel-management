  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 h-fit">
      ...
      <form onSubmit={(e) => { e.preventDefault(); create.mutate({ name }) }} className="flex gap-2 mb-4">
        ...
        <button type="submit" className="w-9 h-9 bg-orange-500 hover:bg-orange-600 text-white rounded-xl flex items-center justify-center transition-colors">
      ...
      <div className="space-y-1">
        {cats?.map(cat => (
          <div key={cat.id} className="flex items-center justify-between px-3 py-2 hover:bg-gray-50 rounded-xl group text-sm">
            ...
            <button onClick={() => del.mutate(cat.id)} className="opacity-0 group-hover:opacity-100 transition-opacity">
...
export default function MenuManager() {
  ...
  return (
    <div>
      ...
      {/* QR ordering toggle */}
      <div className={`flex items-center justify-between rounded-2xl border px-5 py-4 mb-6 ${settings?.qr_ordering_enabled === false ? 'bg-amber-50 border-amber-200' : 'bg-green-50 border-green-200'}`}>
                ...
                {filteredItems?.map(item => (
                  <tr key={item.id} className="hover:bg-gray-50/80 transition-colors">
                    ...
                    <td className="px-4 py-3.5">
                      <div className="flex items-center gap-3">
                        {item.image
                          ? <img src={`/storage/${item.image}`} className="w-9 h-9 rounded-lg object-cover shadow-sm" />
