...
export function RestaurantCard({ restaurant }: { restaurant: Tenant }) {
  const { slug, name, cover_image_url, logo_url, cuisine_type, avg_rating, review_count, price_range, is_open, city, address } = restaurant;
  ...
  return (
    ...
    >
      <div className="relative h-48 bg-stone-100 overflow-hidden">
        {cover_image_url ? (
          <img src={cover_image_url} alt={`${name} cover`} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
        ) : (
        ...
        {logo_url && (
          <div className="absolute bottom-3 right-3 w-10 h-10 rounded-xl overflow-hidden border-2 border-white shadow-md bg-white">
            <img src={logo_url} alt={`${name} logo`} className="w-full h-full object-contain" />
          </div>
        )}
      </div>
      <div className="p-4 space-y-3">
        <div>
          <h3 className="font-semibold text-stone-900 text-base leading-tight group-hover:text-rose-600 transition-colors duration-150">{name}</h3>
