...
export default function GoodbyePage() {
  return (
    <main className="min-h-[85dvh] flex flex-col items-center justify-center px-4 py-16 text-center">
      ...
      <p className="text-stone-500 text-lg max-w-sm mb-2 leading-relaxed">Your table has been closed. We hope you had a great experience.</p>
      ...
      <div className="flex items-center gap-1.5 text-amber-500 mb-10">
        {[1, 2, 3, 4, 5].map((s) => <Star key={s} className="w-6 h-6 fill-amber-400 stroke-amber-400" aria-hidden="true" />)}
      </div>
      <Link to="/"><Button size="lg"><Home className="w-4 h-4" />Explore More Restaurants</Button></Link>
    </main>
  );
}
