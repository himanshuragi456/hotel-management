    ...
    <main className="min-h-[80dvh] flex flex-col items-center justify-center px-4 py-16 text-center">
        ...
        <div className="flex flex-wrap justify-center gap-2 mb-4">
          {orderList.map((num) => (
            <span key={num} className="flex items-center gap-1.5 text-sm font-medium text-stone-700 bg-stone-100 rounded-full px-3 py-1.5">
              <Hash className="w-3.5 h-3.5 text-stone-400" aria-hidden="true" />
              {num}
            </span>
          ))}
        </div>
      ...
      {total > 0 && (
        <p className="text-stone-500 text-sm mb-4">
          Total paid: <span className="font-semibold text-stone-900">{formatCurrency(total)}</span>
        </p>
      )}
...
export default function OrderConfirmedPage() {
  return (
    <Suspense fallback={<div className="min-h-[80dvh] flex items-center justify-center text-stone-400">Loading...</div>}>
