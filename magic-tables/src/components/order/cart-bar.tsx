...
// ─── Cart sheet (locked session — place order directly) ───────────────────────
        ...
        <div className="flex-1 overflow-y-auto space-y-3 mb-4">
          {items.map(({ menuItem, quantity }) => (
            <div key={menuItem.id} className="flex items-center gap-3">
              ...
              <p className="text-sm font-semibold text-stone-900 w-14 text-right tabular-nums">
    ...
    <>
      <div className="fixed bottom-6 left-0 right-0 z-40 flex justify-center px-4 pointer-events-none">
        <button
          ...
          aria-label={`${locked ? "View cart" : "View cart"}: ${itemCount} items, ${formatCurrency(subtotal)}`}
        >
      ...
      {sheetOpen && locked && (
        <LockedCartSheet onClose={() => setSheetOpen(false)} />
      )}
    </>
  );
}
